...
class WeightLoader(object):
    ...
    def __init__(self, weights_file_path):
        ...
        weight_loader = torch.jit.load(weights_file_path).to(device)
        weight_loader.eval()
        all_weight_loader = weight_loader.state_dict().items()
        k_val = {}
        for k1, v1 in all_weight_loader:
            m_num = v1.cpu().numpy()
            k_val[k1] = {
                'data': m_num,
            }
    ...
    def _load_one_param_type(self, conv_params, param_category, suffix):
        ...
        list_str = param_name.split("_")
        i = int(list_str[0]) - 1
        layer_name =""
        if list_str[2] == "conv":
            layer_name =  "layer_" + str(i) + ".0."
        elif list_str[2] == "bn":
            layer_name =  "layer_" + str(i) + ".1."

        if list_str[3] == "scale":
            layer_name += "weight"
        elif list_str[3] == "mean":
            layer_name += "running_mean"
        elif list_str[3] == "var":
            layer_name += "running_var"
        elif list_str[3] == "weights":
            layer_name += "weight"
        else:
            layer_name += list_str[3]
        # a = self.k_val[layer_name]
        param_data = self.k_val[layer_name]['data']
...
class GraphBuilderONNX(object):
    ...
    def build_onnx_graph(
        ...
        all_weight_loader = WeightLoader(weights_file_path)
        initializer = list()
        for layer_name in self.param_dict.keys():
            _, layer_type = layer_name.split('_', 1)
            if layer_type == 'convolutional':
                conv_params = self.param_dict[layer_name]
                initializer_layer, inputs_layer = all_weight_loader.load_conv_weights(
                    conv_params)
    ...
    def _make_yolo_node(self, layer_name, layer_dict):
        ...
        param = {'anchors':anchors,'classes':classes,'down_stride':down_stride,'anchor_num':anhor_num,"infer_thresh":thresh,"gride_w":width,"gride_h":height}
        previous_node_specs = self._get_previous_node_specs()
        inputs = [previous_node_specs.name]
        channels = previous_node_specs.channels
        assert channels > 0
...
def main():
    ...
    argparse.add_argument("--cfg",default="D:\\onnx\\torch_onnx\\yolov3_onnx_test\\yolov3-ocr.cfg")
    argparse.add_argument("--weights",default="D:\\onnx\\torch_onnx\\yolov3_onnx_test\\yolov3_epoch_240_mAP_0.975650_c.pt")
    argparse.add_argument("--out",default="D:\\onnx\\torch_onnx\\yolov3_onnx_test\\yolov3-ocr.onnx")
