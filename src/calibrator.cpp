...
nvinfer1::IInt8Calibrator* GetInt8Calibrator(const std::string& calibratorType,
    ...
    if (calibratorType == "Int8EntropyCalibrator2") {
        return new Int8EntropyCalibrator2(BatchSize, input_w, input_h, img_dir, calib_table_name, input_blob_name, read_cache);
    }
    else if (calibratorType == "Int8MinMaxCalibrator") {
        return new Int8MinMaxCalibrator(BatchSize, input_w, input_h, img_dir, calib_table_name, input_blob_name, read_cache);
    }
    else {
...
Int8MinMaxCalibrator::Int8MinMaxCalibrator(int batchsize, int input_w, int input_h, const char* img_dir, const char* calib_table_name, const char* input_blob_name, bool read_cache)
    : batchsize_(batchsize)
    , input_w_(input_w)
    , input_h_(input_h)
    , img_idx_(0)
...
int Int8MinMaxCalibrator::getBatchSize() const
{
    return batchsize_;
}

bool Int8MinMaxCalibrator::getBatch(void* bindings[], const char* names[], int nbBindings)
{
    if (img_idx_ + batchsize_ > (int)img_files_.size()) {
        return false;
    }
...
const void* Int8MinMaxCalibrator::readCalibrationCache(size_t& length)
{
    std::cout << "reading calib cache: " << calib_table_name_ << std::endl;
    calib_cache_.clear();
    std::ifstream input(calib_table_name_, std::ios::binary);
...
void Int8MinMaxCalibrator::writeCalibrationCache(const void* cache, size_t length)
{
    std::cout << "writing calib cache: " << calib_table_name_ << " size: " << length << std::endl;
    std::ofstream output(calib_table_name_, std::ios::binary);
    output.write(reinterpret_cast<const char*>(cache), length);
